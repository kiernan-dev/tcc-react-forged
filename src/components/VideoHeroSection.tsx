...
const VideoHeroSection: React.FC = () => {
  const [videoStatus, setVideoStatus] = useState<'loading' | 'playing' | 'error' | 'fallback'>('loading');
  const [isLoaded, setIsLoaded] = useState(false);
  const [isMobile, setIsMobile] = useState(false);
  const videoRef = useRef<HTMLVideoElement>(null);

  useEffect(() => {
    // Check for mobile devices - use static background for better performance
    const checkMobile = () => {
  ...
  return (
    <>
      {/* Fixed Video Background for Parallax Effect */}
      {!isMobile && videoStatus !== 'fallback' && (
        <video
          ref={videoRef}
          autoPlay
          loop
      ...
      {/* Fixed Fallback Background Image */}
      {(isMobile || videoStatus === 'fallback' || videoStatus === 'error') && (
        <div 
          ...
          style={{
            ...
            width: '100vw',
            height: '100vh',
            animation: videoStatus === 'fallback' ? 'subtle-zoom 20s ease-in-out infinite alternate' : 'none',
          }}
        />
      )}

    <section id="hero" className="relative min-h-screen flex items-center overflow-hidden z-20"
             style={{ background: 'transparent' }}>
      
      {/* Video Loading Overlay */}
      {videoStatus === 'loading' && !isMobile && (
        <div className="fixed top-0 left-0 w-full h-full bg-black z-5 flex items-center justify-center">
          <div className="text-white text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-amber-500 mx-auto mb-4"></div>
            <p className="text-lg">Loading Background Video...</p>
          </div>
        </div>
      )}
      
      {/* Enhanced Dark Overlay for Text Readability */}
      <div className="fixed top-0 left-0 w-full h-full bg-gradient-to-br from-black/60 via-black/40 to-black/70 z-10" />
        ...
        <div className="flex items-center min-h-screen">
          ...
          <div className={`w-full lg:w-3/5 lg:pr-12 transition-all duration-1000 ${
            ...
            <div className={`mb-8 transition-all duration-1500 delay-500 ${
              ...
              <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-4 text-lg text-gray-200"
                   style={{ 
                     textShadow: '0 0 15px rgba(0,0,0,0.8)',
                     filter: 'drop-shadow(1px 1px 2px rgba(0,0,0,0.9))'
                   }}>
                <span className="hover:text-amber-400 transition-colors duration-300">Custom Builds</span>
                <span className="text-amber-500 animate-pulse hidden sm:inline">•</span>
